...
import { ContainerQueryMixin } from "../../mixins/container-query.js";
...
export class Container extends ContainerQueryMixin(SpectrumElement, {
  ...
  public static get styles(): CSSResultArray {
    const styles = css`
      :host {
        ...
        color: var(
          --ue-container-text-color,
          var(--spectrum-alias-text-color, #212121)
        );
        display: grid;
        gap: var(
          --ue-container-gap,
        ...
        grid-template-columns:
          minmax(1px, var(--ue-container-grid_start-mobile, 2fr))
          minmax(1px, var(--ue-container-grid_center-mobile, 3fr)) minmax(1px, var(--ue-container-grid_end-mobile, 2fr));
        min-height: var(--ue-container-min-height-mobile, 40px);
        padding: var(
            --ue-container-padding-vertical,
            var(--spectrum-global-dimension-size-50, 4px)
          )
          var(
            --ue-container-padding-horizontal,
            var(--spectrum-global-dimension-size-50, 4px)
          );
        width: 100%;
      ...
      :host([desktop-s]) {
        grid-template-columns:
          minmax(1px, var(--ue-container-grid_start, 2fr)) minmax(
            1px,
            var(--ue-container-grid_center, 3fr)
          )
          minmax(1px, var(--ue-container-grid_end, 2fr));
  ...
  public checkVisibility(): void {
    for (const action of ["hide", "show"]) {
      ...
      [...this._slots, ...this._dividers]
        .filter(
          (element) =>
            !!element
              .getAttributeNames()
              .find((attribute) => attribute.endsWith(`:${action}`))
        )
      ...
      this._allAssignedElements
        ...
        .forEach((element) =>
          this.toggleVisibility(element as HTMLElement, action)
        );
    }
  ...
  public toggleVisibility(element: HTMLElement, action: string): void {
    ...
    element
      ...
      .map((rule) => {
        ...
        let hasBreakpoint = this._hasBreakpointByName(
          rule.replace(`:${action}`, "")
        );
  ...
  private _toggleDividers(slot: HTMLSlotElement) {
    const isVisible: Function = (slot: HTMLSlotElement) =>
      slot && !slot.hidden && this._hasVisibleChildren(slot);
    ...
    prevDivider?.toggleAttribute(
      "hidden",
      (!isVisible(slot) && (lastSlot || firstSlot)) ||
        (isVisible(slot) && firstSlot)
    );
